...
// Tune per-feed HTTP timeout. Keep < 8s so the cron function stays well inside
// Vercel's 10s Hobby limit (or 60s Pro limit). All feeds are fetched in parallel
// so total wall-time ≈ slowest individual feed, not the sum.
...
// ─────────────────────────────────────────────────────────────────────────────
// FEED REGISTRY
// Add / remove entries here to change what the digest pulls from.
// Each entry: { url, source, sector, paywall }
// ─────────────────────────────────────────────────────────────────────────────
const FEEDS = [
  // ── Direct RSS feeds ─────────────────────────────────────────────────────
  {
    url: 'https://altassets.net/feed',
    source: 'AltAssets', sector: 'PE', paywall: false,
  },
  {
    url: 'https://www.privateequitywire.co.uk/feed/',
    source: 'PE Wire', sector: 'PE', paywall: false,
  },
  {
    url: 'https://therealdeal.com/feed/',
    source: 'The Real Deal', sector: 'RE', paywall: false,
  },
  {
    url: 'https://www.commercialobserver.com/feed/',
    source: 'Commercial Observer', sector: 'RE', paywall: false,
  },
  {
    url: 'https://renewablesnow.com/news/rss/',
    source: 'Renewables Now', sector: 'Infra', paywall: false,
  },
  {
    url: 'https://opalesque.com/rss.xml',
    source: 'Opalesque', sector: 'Hedge', paywall: false,
  },
  {
    url: 'https://feeds.content.dowjones.io/public/rss/RSSMarketsMain',
    source: 'WSJ Markets', sector: 'Macro', paywall: true,
  },

  // ── Google News keyword searches ─────────────────────────────────────────
  // These proxy Bloomberg, Reuters, FT etc. and are reliable from server-side.
  // Titles arrive as "Headline - Source Name"; we split them below.
  {
    url: 'https://news.google.com/rss/search?q=%22private+equity%22+%22buyout%22+OR+%22fundraising%22+OR+%22lbo%22&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'PE', paywall: false,
  },
  {
    url: 'https://news.google.com/rss/search?q=%22private+credit%22+OR+%22direct+lending%22&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'Credit', paywall: false,
  },
  {
    url: 'https://news.google.com/rss/search?q=%22infrastructure+fund%22+OR+%22energy+transition%22+investment&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'Infra', paywall: false,
  },
  {
    url: 'https://news.google.com/rss/search?q=%22real+estate%22+%22private+equity%22+OR+%22commercial+real+estate%22+fund&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'RE', paywall: false,
  },
  {
    url: 'https://news.google.com/rss/search?q=site:bloomberg.com+%22private+equity%22+OR+%22private+credit%22+OR+%22private+markets%22&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'PE', paywall: true,
  },
  {
    url: 'https://news.google.com/rss/search?q=site:reuters.com+%22private+equity%22+OR+%22private+credit%22+OR+%22hedge+fund%22&hl=en-US&gl=US&ceid=US:en',
    source: 'Google News', sector: 'Macro', paywall: false,
  },
];
...
// Google News RSS appends " - Source Name" to every title. Split it off so the
// digest shows the true publication name rather than "Google News".
...
// Short stable key used for cross-day deduplication (stored in Redis).
// Uses the article headline, not the URL, so the same story discovered via
// two different feeds is still treated as one story.
...
// ─────────────────────────────────────────────────────────────────────────────
// SECTOR DETECTION  (title + summary → sector string)
// ─────────────────────────────────────────────────────────────────────────────
function detectSector(title, summary, defaultSector) {
  ...
  if (/(private equity|buyout|lbo|take.private|growth equity|venture capital|gp stake)/i.test(t)) return 'PE';
...
function scoreArticle({ headline = '', summary = '', source, paywall, pubDate }) {
  let score = 25;
  ...
  // Open vs paywalled (slightly prefer open summaries)
  score += paywall ? -5 : 10;

  // Fund events (highest signal for PE intel)
  ...
  if (fundKw.some(kw => t.includes(kw))) score += 20;
  if (fundKw.some(kw => headline.toLowerCase().includes(kw))) score += 10;
  ...
  if (dealKw.some(kw => t.includes(kw))) score += 15;
  if (dealKw.some(kw => headline.toLowerCase().includes(kw))) score += 10;
...
// ─────────────────────────────────────────────────────────────────────────────
// REDIS DEDUPLICATION  (sorted set keyed by title hash, score = timestamp ms)
// Prevents the same story from appearing in tomorrow's digest.
// ─────────────────────────────────────────────────────────────────────────────
...
async function filterSeen(redis, articles) {
  ...
  try {
    ...
    return articles; // safe fallback: skip dedup rather than block the digest
  }
}
...
async function markSeen(redis, articles) {
  ...
  try {
    const pipeline = redis.pipeline();
    pipeline.zremrangebyscore(SEEN_KEY, 0, cutoff); // prune stale entries
    articles.forEach(a => pipeline.zadd(SEEN_KEY, { score: now, member: titleHash(a.title) }));
    await pipeline.exec();
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN EXPORT
// Returns the top `count` articles ready to drop into buildDigestHtml().
// ─────────────────────────────────────────────────────────────────────────────
async function fetchTopArticles(redis, count = 5) {
  // 1. Fetch all feeds in parallel; tolerate individual feed failures
  const results = await Promise.allSettled(FEEDS.map(fetchFeed));
  let pool = results.flatMap(r => (r.status === 'fulfilled' ? r.value : []));

  // 2. In-memory dedup: same URL OR same normalized title from two feeds
  ...
  // 3. Freshness filter: prefer last 48 h; expand windows if needed
  const now = Date.now();
  const H48 = 48 * 3_600_000;
  const H7D = 7 * 24 * 3_600_000;
  let fresh = pool.filter(a => a.pubDate && now - a.pubDate.getTime() <= H48);
  if (fresh.length < count) fresh = pool.filter(a => a.pubDate && now - a.pubDate.getTime() <= H7D);
  if (fresh.length < count) fresh = pool; // total fallback (undated items)

  // 4. Cross-day dedup via Redis
  const unseen = await filterSeen(redis, fresh);
  const candidates = unseen.length >= count ? unseen : fresh; // fallback if all seen

  // 5. Score and select top N
  const scored = candidates
    .map(a => ({ ...a, score: scoreArticle(a) }))
    .sort((a, b) => b.score - a.score)
    .slice(0, count);

  // 6. Shape output for buildDigestHtml
  return scored.map(a => ({
    sector: a.sector,
    headline: a.title,
    summary: a.summary,
    whyItMatters: whyItMatters(a),
    source: a.source,
    url: a.url,
    pubDate: a.pubDate,
